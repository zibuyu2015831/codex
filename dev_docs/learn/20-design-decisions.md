...
**代价**：编译产物大（几十 MB 起）；启动时要判断"我是谁"；理想守不到 100%（codex 随发布另交付 7 个辅助可执行文件）。
...
| 代价 | 必需的应对 |
| ---- | ---- |
| 调试变难（没有调用栈） | **结构化追踪不是加分项，是必需配套**（见下） |
| 类型爆炸 | 强类型语言靠穷尽匹配；弱类型靠注册表 + 测试 |
| 时序问题 | 每个事件要设计幂等性 |

> **`结构化追踪`（structured tracing）**：**日志不再是一行行孤立的文本，而是每条都自动带上"我属于哪次请求、哪个会话、哪个执行区间"**，从而能把散落的日志按因果关系串回去。
>
> 为什么它在这个架构里是必需的：**函数调用出错时你有调用栈，一眼能看到"谁调了谁"；消息传递没有调用栈**——你只看到"发出去一条""收回来一条"，中间的因果链断了。结构化追踪就是用来补上这条链的。codex 的做法是每次提交开一个 **span**（[03](./03-protocol.md) §6），这一段里的所有日志自动带上同一个会话 ID。
...
| **双构建系统**（Cargo + Bazel） | 发布要可复现、要**交叉编译**（在一台 Mac 上编出 Linux/Windows 能跑的程序） | 纯粹的负担 |
