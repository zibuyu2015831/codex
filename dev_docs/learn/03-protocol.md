...
> **`token`（词元）在本教程里不译**——模型不是按字符也不是按单词处理文本的，而是先切成一个个小片段，每片叫一个 token（英文约 4 个字符、中文约 1 个汉字算一个）。**它是这个领域事实上的计量单位：模型按 token 计费、按 token 限长。** [08](./08-context.md) §1 会展开，那一篇整篇都在处理"token 不够用了怎么办"。
...
> **`stdio` 就是 `stdin` + `stdout` 这一对**（[01](./01-coordinates.md) §2 ③ 讲过）。"用 stdio 当传输通道"的意思是：**两个进程之间不开网络端口，直接一个把消息写进对方的标准输入、另一个从自己的标准输出写回去**。IDE 插件拉起 codex 子进程就是这么通信的——**不占端口、不需要鉴权、进程一关连接自然就断**，是本机进程间通信最省事的一种做法。
