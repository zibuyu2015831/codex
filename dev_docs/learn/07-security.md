...
> **图上有两个词先解释掉：**
>
> **`Guardian`（守卫）是 codex 自己的一个组件名**，不是通用词。它干的事是：**在"该不该批准这条命令"这个问题上，用一个自动评审顶替人工点头**——你可以配置成"由它判断，它说行就放行"，这样无人值守跑批时不至于卡在等人点击上。所以图上写的是"走 Guardian 自动评审**而非**问人"，两者是替代关系。
>
> **`启发式`（heuristic）** 是个常见但容易劝退的词，意思很朴素：**一套"多半准、但不保证准"的经验判断规则**。和它相对的是"能证明一定正确的判定"。图里那句"启发式判定这次失败是不是被沙箱拒的"，说的就是：操作系统并不会明说"我是因为沙箱才拒你的"，只给一个普通的失败码，所以 codex 只能**根据错误的特征去猜**。**猜错是可能的**——这正是它被叫作启发式而不是"判定"的原因。
...
> **`系统调用`（syscall）是这一段的关键词。** 程序自己其实什么都干不了——**打开文件、发网络包、创建进程，全都得请操作系统代劳**，请求的方式就是发起一次系统调用。可以理解成程序和操作系统之间**唯一的那道窗口**。
>
> 所以"seccomp 做系统调用过滤"的含义就非常硬：**它守在那道窗口上，规定这个进程只准递交哪些请求**。递交了不在清单里的，操作系统直接驳回。**这比"检查命令字符串里有没有 `rm -rf`"强一个量级**——后者能被各种花样绕过，前者绕不过去，因为绕过它就等于绕过操作系统本身。

bwrap 二进制是**仓库内 vendored 的 bubblewrap C 源码**编译出来的，随 Linux 发布包一起分发。

> **`vendored`（直译"供应商化"）指的是：把一份第三方代码，直接复制一份进自己的仓库跟着一起编译**，而不是在清单文件里写"我依赖某某库某某版本"让构建时去下载。
>
> 好处是**构建结果完全可控**——不怕对方删版本、改内容、断网。代价是升级要自己手动同步，而且**对方的许可证义务跟着这份复制品一起进了你的仓库**（[21](./21-before-you-fork.md) §2 就栽在这一点上：bubblewrap 是 LGPL，和仓库其余部分的 Apache 2.0 不是一回事）。
...
> **"正交"（orthogonal）借自数学里的"互相垂直"，在工程语境里意思是：两件事互不影响，可以自由组合。**
>
> 说"文件系统权限和网络权限是正交的"，就是说：**给不给文件权限，和给不给网络权限，是两个独立的旋钮**——四种组合（都给 / 都不给 / 只给文件 / 只给网络）全都是有意义的真实场景。下面那张表就是在证明这一点。
>
> **反面是"耦合"**：如果做成一个"安全等级：低/中/高"的单滑块，你就只能得到一条对角线上的三种组合，"可写文件但禁网"这种最常用的档位反而表达不出来。
...
> **"纯函数"是说：这个函数只根据传进去的参数算出一个返回值，除此之外什么都不干**——不写日志、不改全局变量、不碰磁盘、不发网络。同样的输入永远给同样的输出。
>
> 上面那句英文注释里的 "side-effect free"（无副作用）就是这个意思。**纯函数的好处是极其好测、好复用**：你可以随便调它一百次，不会在系统里留下任何痕迹。所以 codex 才把"记录"这件有副作用的事，从判定里剥出去交给调用方。
