...
> **图里的 `turn` 是本教程后面反复出现的核心词，先定义清楚。**
>
> **一个 turn = 你说一句话之后，智能体"想一下 → 干点活 → 再想 → 再干"，直到它认为这件事办完为止的整个过程。**
>
> 注意它**不是**"问模型一次"。**一个 turn 内部可能问了模型十几次、跑了几十条命令**——每次模型回话说"我还要看看某个文件"，就再问一次。turn 结束的标志是模型不再要求做任何事了。
>
> 中文没有特别顺口的对应词（"一轮"容易和"问模型一次"混淆），所以本教程**直接沿用 `turn`**。看到它就想："用户一句话引发的完整一次办事过程"。
...
> **`drop` 是 Rust 里高频出现的一个词**，意思是"**这个值的最后一个持有者消失了，于是它被自动销毁**"（相当于 Python 里对象引用计数归零被回收，只是 Rust 里这件事发生的时刻是确定的、可预测的）。
>
> 具体到这里：管子的发送端 `tx_sub` 握在前端手里。前端一没了，`tx_sub` 就被销毁，channel 也就自动关闭——**这个"关闭"没有任何人显式发指令，是所有权规则的副产品**。所以内核这边只能通过"`recv()` 开始返回 `Err`"察觉到它。
...
收到活儿之后，包装成一个 `SessionTask` 丢到后台跑。这个 **trait**（就是"接口"，见 [01](./01-coordinates.md) §2 补课）定义在 `codex-rs/core/src/tasks/mod.rs:184`：
...
> **先扫掉第一行那串 `: Send + Sync + 'static`。** 它们是三条**约束**，写在这里表示"想当 `SessionTask` 的类型，必须同时满足这三条"：
>
> | 约束 | 大白话 |
> | ---- | ---- |
> | `Send` | 这个东西**可以被搬到另一个线程上去** |
> | `Sync` | 这个东西**可以被多个线程同时读** |
> | `'static` | 这个东西**不借用任何有期限的外部数据**，所以能活任意久 |
>
> 三条加起来就一句话：**这个任务要被丢到后台的线程池上跑，且不知道会跑多久，所以它必须是"扛得住这种折腾"的类型。** Python 里没有对应写法，**根子上是因为 asyncio 的协程只跑在一个线程上，压根不存在"被搬到另一个线程"这件事**（见 [01](./01-coordinates.md) §2 补课里 tokio 与协程的对照表）；tokio 默认是多线程的，于是这些检查就必须写出来。[32](./32-rust-for-python-readers.md) §1、§4 有展开。
