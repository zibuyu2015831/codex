...
> **先说清"工具规格（spec）"是什么**，后面几节都要用：**它就是你给模型的那份说明书**——这个工具叫什么名字、是干什么的、要传哪几个参数、每个参数是什么类型。模型正是**看着这份说明书**才知道有哪些工具可用、该怎么填参数。
>
> 这份说明书的格式通常是 **JSON Schema**——一套用 JSON 来描述"另一段 JSON 该长什么样"的通用写法（`"type": "string"`、`"required": ["command"]` 之类）。它不是 codex 发明的，各家模型的工具调用接口基本都认这个。
...
> ⚠️ 上面三类是**快速定位用的启发式**（**"启发式"= 一套多半准、但不保证准的经验判断**），**不是穷举分类**。动态工具和运行期注册的规格都可能落在三类之外。遇到对不上号的工具名，回到 `codex-rs/core/src/tools/registry.rs` 追注册路径。
