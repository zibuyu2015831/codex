...
**③ 这些行数里有很大一块是测试，不是你要读懂的东西。** 以 core 为例：

| 部分 | 行数 | 占比 |
| ---- | ---: | ---: |
| `codex-rs/core/src/`（生产代码） | 185,107 | 62% |
| `codex-rs/core/tests/`（集成测试） | 111,856 | **38%** |
| 合计 | 296,963 | |

> 复现：`find codex-rs/core/src -name '*.rs' -exec cat {} + | wc -l`，`tests` 同理。两数相加与 `git ls-files codex-rs/core | grep '\.rs$' | xargs wc -l` 的总数完全一致。
>
> **对心理预期的影响**：看到"29.7 万行内核"会劝退，但你真正要理解的是那 18.5 万行；测试是**跟着功能走**的，删功能时一并删（见 [22](./22-load-bearing-and-cuts.md) §5 — 真正的痛点：`core` 的顶层是一片平地）。
...
> **`vendored` 指的是"把一份第三方代码直接复制进自己仓库跟着一起编译"**，而不是在清单文件里声明依赖、构建时去下载（[07](./07-security.md) §5 有展开）。
>
> **这里的要害正在于此：代码进了你的仓库，对方的许可证义务也就跟着进来了。** 所以同一个仓库里会出现两种许可证——根目录是 Apache 2.0，`vendor/bubblewrap/` 那一块是 LGPL。
...
> ⚠️ **最后一条最容易漏**：不改**遥测**端点 = 你的用户数据往上游服务器发（**遥测 = 程序自动上报自己的运行情况**，见 [01](./01-coordinates.md) §3）。这既是合规问题也是隐私问题。
...
>
> **`core` 内部要分两半看**：17 个子目录（13.4 万行）可以按目录整块切；顶层 112 个平铺文件（5.1 万行）没有分组，只能从 `Op` 入口逐个倒推。后者是 fork codex 最大的技术痛点，见 [22](./22-load-bearing-and-cuts.md) §5 — 真正的痛点：`core` 的顶层是一片平地。
...
| 最麻烦的是哪？ | **`core` 的顶层平地**——112 个无分组文件（5.1 万行）。它的 17 个子目录（13.4 万行）反而好切 |
