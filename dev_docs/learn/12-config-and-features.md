...
> | 词 | 一句话 |
> | ---- | ---- |
> | **`TOML`** | 一种配置文件格式，`键 = 值` 加上 `[小节名]` 分组。**比 YAML 严格、比 JSON 好写**，是 Rust 生态的默认选择——每个 crate 的清单文件用的就是它 |
> | **`profile`（配置档）** | **一组起了名字的配置组合**，可以整套切换。类似"工作模式"和"在家模式"——`-p work` 就切到 `work` 那一整套设置，不用逐项改 |
> | **`MDM`** | Mobile Device Management，**企业用来统一管控员工设备的系统**。公司 IT 通过它下发策略到每台机器上，员工改不了。表里那两个"遗留 MDM"层就是这条通路下来的配置 |
