...
summary: 描述 codex-app-server-protocol 的 v1/v2 分版结构、方法注册表所在的四个宏与按命名空间的真实方法分布（218 个协议名）、Rust 类型作为唯一事实源与 ts-rs 导出 550 个 TypeScript 类型的生成链路、schema fixtures 的真实再生成入口（justfile 里的 recipe 已失效）、AGENTS.md 中的 API 设计硬性规则，以及协议变更的高风险提示。
keywords: codex | app-server | json-rpc | protocol | ts-rs | schema | api-versioning | method-registry
scope: codex-rs/app-server-protocol 与 codex-rs/app-server 的对外协议
related_files: codex-rs/app-server-protocol/src/rpc.rs | codex-rs/app-server-protocol/src/protocol/common.rs | codex-rs/app-server-protocol/src/protocol/v2/mod.rs | codex-rs/app-server-protocol/src/protocol/v2/permissions.rs | codex-rs/app-server-protocol/src/protocol/v2/config.rs | codex-rs/app-server-protocol/src/experimental_api.rs | codex-rs/app-server-protocol/Cargo.toml | codex-rs/app-server-protocol/scripts/write_schema_fixtures.py | codex-rs/app-server/Cargo.toml | justfile | AGENTS.md
...
> **证据等级**: 方法清单为 E3（从注册表宏抽取）；目录与文件数为 E1；生成链路为 E2（规范/脚本明文）+ E3（源码）
...
| 加上无 wire 名的 `ClientNotification::Initialized` | **218** | E3 |
| 生成的 v2 TypeScript 类型文件 | 550 | E1（`git ls-files`） |
| `schema/typescript/` 全部文件 | 642 | E1 |
| `schema/json/` 文件 | 285（递归口径，见 §7） | E1 |
| `schema/precomputed/` 文件 | 2 | E1 |
| `protocol/v2/` 下的 Rust 模块（不含测试） | **28** | E1（`ls` + `mod.rs` 声明） |
...
> [!IMPORTANT]
> `src/rpc.rs:1-2` 的模块文档第一句就是：
>
> > "We do not do true JSON-RPC 2.0, as we neither send nor expect the `\"jsonrpc\": \"2.0\"` field."
>
> 文件里确实定义了 `pub const JSONRPC_VERSION: &str = "2.0";`（`rpc.rs:11`），但那不代表线上帧里带这个字段。**写第三方客户端时不要按 JSON-RPC 2.0 规范去校验帧格式。** 本文其余部分沿用「JSON-RPC」这个说法，但请记住这个限定。
...
| `server_notification_definitions!` | `:1684-1787` | `enum ServerNotification` | 72（其中 1 条不用 `=> "..."` 而用 rename/strum 属性声明，纯箭头抽取只得 71） |
| `client_notification_definitions!` | `:1808-1810` | `enum ClientNotification` | 1（`Initialized`，无 wire 名） |
...
> [!NOTE]
> **为什么要匹配 `strum(serialize = ...)` 而不只是 `=> "..."`**：绝大多数条目写成 `Variant => "ns/method" { ... }`，但 `AccountLoginCompleted`（`common.rs:1782-1785`）改用 `#[serde(rename)]` + `#[ts(rename)]` + `#[strum(serialize)]` 三件套声明 wire 名。只抓 `=> "` 会把它漏掉，`account` 就会少算成 12。
...
> [!NOTE]
> **勘误：举例举错了区段。** 上一稿把 `ExecCommandApproval` 当作 `client_request_definitions!` 里的 v1 例子——**它不在这个宏里**。`ExecCommandApproval { params: v1::ExecCommandApprovalParams }` 位于 **`server_request_definitions!`**（`common.rs:1529` 起，该条目在 `:1596-1597`），即**服务端发给客户端**的请求，方向与 `ClientRequest` 相反。
>
> **"4 处"这个计数是对的**，只是四个成员全部是上面列出的那些。`server_request_definitions!` 里另有 2 处 v1（`ApplyPatchApproval` `:1591`、`ExecCommandApproval` `:1597`）与 9 处 v2，属于另一张表。
>
...
前 5 个是请求，后 5 个是通知。它们早于 `<resource>/<method>` 约定，新代码不应仿照。
...
> - `windowsSandbox` 从「各 3」一行中被单独确认存在（第一版 §5 的模块表却漏了 `windows_sandbox.rs`，前后矛盾）
> - **删除三个不存在的命名空间**：`tmp`（`common.rs:2933` 等测试里的路径字面量 `tmp/AGENTS.md`）、`plugins`（测试用例名）、`openai`（`model_provider: "openai"` 这个**类型判别值**）
> - `enum/unit`、`enum/tuple`、`enum/named`（`experimental_api.rs:69-73`）同理，是 `#[experimental(...)]` 宏的**单元测试夹具**，不是协议方法
...
> [!IMPORTANT]
> 第一版称命名约定「未在代码中找到明文规定」——**错了**。`AGENTS.md` 的 `## App-server API Development Best Practices` 下有一整套硬性规则，**但它们分散在两个不同的小节里，作用域也不同**（上一稿把全部 8 条都记在 `### Core Rules` 名下，其中 2 条归错了小节）：
>
...
> | wire 上一律 camelCase（字段与字符串枚举值） | `Always expose fields as camelCase on the wire` |
> | **例外**：config RPC 载荷用 snake_case，以对齐 config.toml 键名 | `Exception: config RPC payloads are expected to use snake_case` |
> | v2 载荷字段**禁止** `#[serde(skip_serializing_if = "Option::is_none")]` | `Never use #[serde(skip_serializing_if = ...)] for v2 API payload fields` |
...
> | 规则 | AGENTS.md 关键词 |
> | ---- | ---- |
> | `#[ts(optional = nullable)]` **只能**用在 client→server 的 `*Params` 上 | `Do not use #[ts(optional = nullable)] outside client->server request payloads` |
...
> [!WARNING]
> **反例：三个方法用了 snake_case，违反 camelCase 规则。**
...
> [!NOTE]
> **勘误**：第一版说 27 个模块，且表里漏了 `windows_sandbox.rs`、多列了一行不存在的文件 `skills（见 thread）`。`skills/*` 是 4 个协议方法（`skills/list`、`skills/changed`、`skills/config/write`、`skills/extraRoots/set`），但 v2 目录下**没有** `skills.rs`；而 `windowsSandbox/*` 的 3 个方法**确实**有对应的 `windows_sandbox.rs`。用 `ls codex-rs/app-server-protocol/src/protocol/v2/` 可以一眼复核。
...
> [!NOTE]
> **勘误**：第一版写「58 个 workspace 依赖，全仓第 2」，这个数字复现不出来。按明确口径重新统计 `codex-rs/*/Cargo.toml` 的 `[dependencies]` 段（E4）：
>
...
> [!NOTE]
> `AGENTS.md` 该节的适用文件清单里写的是 `app-server-protocol/src/protocol/v2.rs`（单文件），而实际早已是 `protocol/v2/` 目录——这是上游文档的又一处陈旧，不影响规则本身的效力。
...
| `experimental_api.rs` 的标注机制 | E1（只确认了 `enum/*` 是其测试夹具） | 该文件 + `codex-experimental-api-macros` |
