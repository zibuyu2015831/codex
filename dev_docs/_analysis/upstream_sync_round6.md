...
批次 0–9 全部提交后做的一次独立复查，目标是「确保无误」。门禁三项仍全绿（脱敏 7/7、引用 ERROR 0、跨文档无不一致、账本 33/33 对仓库真值成立），另查出并修复 **3 处门禁查不到的缺陷**，全部落在 `AI_Coding_Context.md`：

| # | 缺陷 | 性质 | 修复 |
| ---: | ---- | ---- | ---- |
| 1 | §「阅读前必读」给读者的纯文档分支复核命令用 `git merge-base HEAD origin/main` 取基线 | **文档给的复核命令本身失效**（与批次 3 那条同型，但这次出在本体系的入口处）。`origin` 是个人 fork，其 `main` 镜像落后实际并入的主分支 2,230 个提交，基线退回合并之前，命令输出 **6,091** 而非 `0`——一个会让读者以为红线已被破坏的假警报 | 改用「最近一次合并提交的第二父节点」取基线（`git rev-parse "$(git rev-list --merges -1 HEAD)^2"`），不依赖任何 remote 的新鲜度；并就地写明为什么不能用 `origin/main` |
| 2 | 禁忌表 **T2** 写「`Op` 枚举真值 26」，并给出交叉验证命令标注「同为 26」 | 该案例是**上一轮**的历史教训，但数字未加时间限定，与账本 `op_variants = 28`（本轮真值，已实测）直接冲突；读者跑那条命令会得到 28，与相邻文字矛盾 | 数字明确标注为「当时基线下的值」，交叉口径的结论改为「与主口径逐字符一致」（不再钉死具体数）。同行的 `thread/*` 57→60 同样加时间限定；其引用的三个方法名经核实今天仍存在，案例本身依然成立 |
| 3 | 自身约束第 23 条写「推送只能推个人 fork，**禁止推上游 `origin`**」 | **内部矛盾**：本仓库 `origin` 正是个人 fork，`upstream` 才是 `openai/codex`。这是 remote 重整（`a1c52a4d2e`）之前的旧命名残留——`AI_RULES.md` §5.1 当时已改对，主文档这条漏改 | 改为「禁止推上游 `upstream`」，补上 push URL 已被置为无效值的事实与 `git remote -v` 先验要求，并链到 `AI_RULES.md` §5.1 |

三条的共同形态值得单列：**它们都不是断言错了，而是「给读者的操作指令」错了**——复核命令的基线、交叉验证命令的期望值、remote 的名字。门禁检查的是文档引用的**路径与数字**，对文档里的**命令语义**完全无感。批次 3 已经记过一次同型教训（`include_disabled`），本轮它在入口文档再次出现，说明：

> **凡文档中出现可执行命令，都必须在定稿前实际跑一遍并比对输出**，而不只是检查它引用的符号是否存在。这一条应当加入下轮的门禁设计或检查清单。

### 仍然未做的事

- ~~**批次 10 的复核轮未执行**~~ —— **已于 2026-09-21 执行，见 §9。**原文保留：批次 0–1 是在单人分轨（§2 已声明的方法论折价）下产出的，至今没有跑过重推导轨。`5052c73ab2`（我自己的 `mcp-server` 能力去向误判）与本节第 1、3 条都是这一折价的实证：前者是断言错误，后者是操作指令错误，**两类都不是门禁能拦下的**。
- `dev_docs/diagrams/` 的五张架构图未按新结构重绘。
- `tui_guide.md` / `experimental_surfaces.md` 本轮只做了数值与失效引用同步，未做深度重核。
- `sdk_guide.md` 的 Python SDK 发布作业清单未重推导（已就地标注）。

---

## 9. 批次 10：独立复核轮（2026-09-21，**已执行**）

§2 声明批次 0–1 是在**单人分轨**（方法论折价）下产出的，§8 把批次 10 列为「⬜ 未执行」。本节记录它的实际执行与结果。

### 9.1 方法

派三个**盲推导代理**（`general-purpose`，均明确要求「不要派生任何子代理」，避免此前 503 并发放大），条件是 **禁止读取 / grep / ls `dev_docs/`**，只从源码与 git 历史独立推导同一批事实：

| 代理 | 覆盖面 | 对应批次 |
| ---- | ---- | ---- |
| crate 全景 | workspace 规模、代码量、依赖度、特定 crate 存在性 | 批次 1 的 `crate_map.md` |
| 架构与入口 | bin 目标、发布交付、子命令、前端、协议核心、AGENTS.md 可解析性 | 批次 1 的 `architecture_overview.md` |
| 事实基座 | 账本 33 条断言的题面（**不给答案**，只给问题） | 批次 0 |

随后由我执行**修订轨**：每一处冲突都回源判定，推导轨的结论不自动采信。

### 9.2 判定结果

**账本 33 条中 29 条逐条吻合**，`crate_map.md` 的 A/B 段全部复算一致（154 / 149 / 26 / 15 / 11、4,631 文件、1,858,112 行、core 402,720、tui 405,863、config 29,374 / 29,515），两条关键结构结论被独立复现：`codex-voice-host` 零库依赖者、`guardian-reviewer` 住在 `ext/` 依赖 `extension-api` 却无 `install`。

**推导轨被驳回 1 处**：它指出 `voice_and_realtime.md` 把进程拉起点引成 `codex-rs/realtime-webrtc/src/session.rs:177-181`（真实 spawn 在 `codex-rs/realtime-webrtc/src/client.rs:158` 与 `:183`）。回源后**判定文档正确**——该文 `:49` 同时列了两个位置，`:112` 引 `codex-rs/realtime-webrtc/src/session.rs:177-181` 说的是逐平台二进制名与 GStreamer 库名配对表，本就不是 spawn 点。

**查出并修复 9 处缺陷**，按形态分三类：

#### 类型一：正文更新了、附录/图示没更新（体系内自相矛盾）

| # | 缺陷 | 说明 |
| ---: | ---- | ---- |
| 1 | `mcp_and_extensions.md` §3.1 整节陈旧 | 装配表引用了**两个全仓零命中的符号**——`AgentSpawner`（能力 trait 已从 `extension-api` 删除）与 `install_executor_plugins`（真名 `install_plugins`），crate 名还停在已改名的 `codex_guardian`；「依次安装 8 个扩展」实为 **10** 个（漏 `queue` 与 `history_notes`）。**批次 1 早已独立发现 `AgentSpawner` 被删**并写进 `crate_map.md` §3.8，但那条发现没有回灌到本节 |
| 2 | `mcp_and_extensions.md` §1.3 关系图陈旧 | 图里画着已删除的 `core-skills` 与 `mcp-server`、已拆分的 `ext/guardian`；「11 个具体扩展中的 8 个实现它」两个数都错（真值 14 个中的 11 个）。**同篇 §1.1 的表格早已更新到 11，图还停在 8** |
| 3 | `mcp_and_extensions.md` §1 修订说明自我矛盾 | `:23` 与 `:73` 明写「core 生产依赖里没有注册扩展」**已被证伪**，`:251` 却把这句话当成「准确表述」重申。同处还把 core 的 `ext/*` 生产依赖数写成 2（真值 **4**：`extension-api` / `items` / `guardian-reviewer` / `skills`）且行号漂移 |
| 4 | `core_agent_loop.md:918` 附录 | 写 `Op` **26** / `EventMsg` **80** 与旧行号，而同篇 `:220` 早已是 **28 / 83** |
| 5 | `AI_Coding_Context.md` 的三处扩展计数 | frontmatter 摘要、关键目录速查表、未闭合清单都停在 8，与体系内 `guardian.md`（写 10，正确）矛盾 |

> **这一类是本轮的主要发现，共 5 处，全部是「同一份文档内部或体系内两篇之间互相矛盾」。** 它们的共同成因是**修订以「节」为单位而非以「事实」为单位**：某轮重写了 §1.1 就以为这个数字改完了，而同一个数字还躺在 §1.3 的 ASCII 图里、§3.1 的表格里、附录的「已覆盖内容」行里。**门禁对此完全无感**——两个矛盾的数字各自都能解析、各自都不在跨文档事实正则的口径内。

#### 类型二：「我发现上游坏了」这个结论过期了

| # | 缺陷 | 说明 |
| ---: | ---- | ---- |
| 6 | 禁忌表 **T4** 的第一条实例已失效 | 上一稿说「`just write-app-server-schema` 跑不通（`codex-app-server-protocol` 无 bin target），而 AGENTS.md、justfile recipe、校验测试的 `panic!` 文案都在推荐它——**循环陈旧**」。实测该 recipe 现为 `justfile:177-178` 的 `{{ python }} app-server-protocol/scripts/write_schema_fixtures.py`，**脚本存在、跑得通**。上游修好了，而本体系那段勘误比它所勘的缺陷活得更久 |

> 已就此新增禁忌 **T7**：**缺陷会被修复，勘误却不会自动撤销。** 写「上游这里是坏的」比写「上游这里是这样的」更需要每轮重跑——因为它的反面（被修好）**不产生任何冲突信号**：引用还在、符号还在、门禁照样绿。这与 §8 第 1 条（复核命令基线失效）同属「门禁看不见命令语义」。

#### 类型三：口径标签与命令实测不符

| # | 缺陷 | 命令实际测的 | 原先声称的 |
| ---: | ---- | ---- | ---- |
| 7 | 账本 `config_schema_keys = 100` | `['properties']` 的项数 | 「100 个**顶层键**」——JSON 根对象顶层键其实只有 **7** 个，`definitions` 另有 180 |
| 8 | 账本 `ts_v2_schema_files = 631` | `v2/` 下**全部** `.ts` | 「**ts-rs 导出** 631 个类型文件」——其中 1 个是不定义类型的 barrel 入口文件，带 ts-rs 横幅的是 **630** |
| 9 | 账本 `tokio_spawn_sites_core_tui = 166` | 只排除文件名 `*_tests.rs` | 「**非测试代码**中 166 处」——未排除 `tests/` 目录，也未排除 `tests.rs` / `test_support.rs` 这类命名的测试辅助文件<!-- ref-exempt: 泛指一类文件名形状，不指某个具体文件 -->。收紧路径过滤后为 **146**（已改为此口径），再排除生产文件内的 `#[cfg(test)]` 内联块与注释行约为 **138** |

> 三条的数字**都是可复算的**，错的是**贴在数字上的标签**。`--verify-repo` 只检查「命令输出 == expected」，对「claim 的措辞是否如实描述这条命令」无从检查。**账本的断言文字必须复述命令的口径，而不是复述我希望它测的东西。**

### 9.3 另外两条不改文档、但要记下的事

1. **`shipped_aux_binaries = 7` 的判据有已知盲区。** `architecture_overview.md` §3.4 原写「唯一可靠的判据是是否出现在发布工作流的 `binaries` 列表里」。实测 `codex-voice-host` 在 `rust-release.yml` 里出现 19 次却**不在任何 `binaries:` 列表中**——它由 Bazel 构建，经 `third_party/voice/assemble_package.py:102` 放进包内的 `codex-resources/voice/bin/`。**反向推断「不在列表 ⇒ 不交付」不成立。** 已就地加警告框；主口径不变（语音包是否为默认发布产物尚未取证）。
2. **两个盲推导代理都在同一个查询上发生了 `dev_docs/` 污染**，且都主动披露：它们用 `grep -rn ... . | grep -v './dev_docs/'` 过滤，而 grep 输出的前缀是 `dev_docs/` 而非 `./dev_docs/`，共约 9 行文档内容进入上下文。两者随后都改用白名单路径 / `--exclude-dir` 重跑。**判定**：mcp-server 一节的**客观证据**（`git log --diff-filter=D`、删除提交的 stat、符号零命中）不受影响；但「能力未迁移」这条**解释性结论**的独立性在形式上已受损，**本轮不把它记为独立佐证**。该结论仍成立，依据是 `5052c73ab2` 当时的独立取证。

### 9.4 批次 10 之后仍未做的事

- `dev_docs/diagrams/` 的五张架构图未按新结构重绘。
- `tui_guide.md` / `experimental_surfaces.md` 仍只做过数值与失效引用同步，未做深度重核。
- `sdk_guide.md` 的 Python SDK 发布作业清单未重推导。
- 本轮只对**批次 0–1 的题域**重跑了推导轨；批次 2–9 各篇的正文未再独立复核——但类型一的 5 处缺陷说明，**「某篇已在某轮重写过」不等于该篇内部已自洽**。
