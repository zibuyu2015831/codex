...
summary: 描述 codex-tui 的模块版图与高触碰大文件实测规模、codex-tui 不「直接」依赖也不「直接」导入 codex-core 这条 CI 强制的架构不变量（core 仍会传递性链接进来，配置类型经 legacy_core 再导出）、styles.md 与 clippy.toml 的样式约定、AGENTS.md 中 TUI 与快照测试条款的落地要点，以及 app / chatwidget / bottom_pane 三层结构与在该 crate 内新增代码的硬性约束。
...
这是本文最容易被误判的一条，也是唯一一条被 CI 脚本单独校验的架构约束。**注意"直接"这两个字——它是约束的全部内容。**
...
2. `codex-rs/tui/src/` 下不出现 `codex_core::` / `use codex_core` / `extern crate codex_core`。

`.github/workflows/repo-checks.yml` 直接 `run: python3 .github/scripts/verify_tui_core_boundary.py`，因此这是 **PR 级别的硬门禁**，不是风格建议。

TUI 的运行时通路是 **app-server 客户端**：`codex-rs/tui/Cargo.toml` 依赖 `codex-app-server-client`，`codex-rs/tui/src/lib.rs` 导入并使用 `InProcessAppServerClient` 与 `RemoteAppServerClient` 两种客户端——前者在同进程内拉起 app-server，后者连接外部 app-server 进程。两条路径共用同一套协议。
...
> - `codex-rs/tui/src/lib.rs:41` `use codex_cloud_config::cloud_config_bundle_loader_for_storage;`、`:66-67` `use codex_utils_oss::{ensure_oss_provider_ready, get_default_model_for_oss_provider};`；
> - `codex-app-server-client` 自身也直接依赖 `codex-core`。
>
> E4 复核（`cargo metadata` 上按 normal 依赖做 BFS）：`codex-tui` 的直接普通依赖里没有 `codex-core`，但沿普通依赖可达，最短路径之一是 `codex-tui → codex-app-server-client → codex-core`。
>
...
>
> 本文档体系第一版把这条边界写反了（说所有前端都汇聚到 codex-core）；第二轮改成「必须先有协议方法」又矫枉过正，漏了这个逃生舱；第三轮补上了逃生舱，却把"不直接依赖"扩大成了"不链接"（见 §0.1 的 CAUTION）。**这个来回本身就说明：涉及依赖边界的断言，务必区分「直接依赖 / 直接导入 / 传递链接 / 再导出」四种关系，并用 `cargo metadata` 而不是 `Cargo.toml` 目视来验证传递关系。** 以 §0.1 + 本节为准。
...
├── wrapping.rs                 ← 换行助手（word_wrap_line / word_wrap_lines）
├── render/line_utils.rs        ← 行前缀助手（prefix_lines）
│
├── 渲染辅助
...
被点名的 5 个高触碰文件（grep `high-touch files`），以及本 crate 的实测 Top 12（E1，`wc -l`）：
...
这不是「TUI 的另一条可选路径」，而是 **TUI 访问会话能力的唯一路径**（见 §0.1）。
...
> **未验证**（E1）：走 in-process 还是 remote 的具体判定条件（命令行参数、配置项还是环境）。读 `codex-rs/tui/src/lib.rs` 中两个客户端的构造点及其上游分支可确认。
...
- [app-server 协议](./app_server_protocol.md) — TUI 唯一的会话通路
